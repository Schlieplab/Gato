...
class Moat:
    def __init__(self, graph_display, pos, embed_radius, color, vertex_num, moat_id):
        d = (embed_radius * graph_display.zoomFactor ) / 100.0 #Canvas radius!
        self.d = d
        self.moat_id = moat_id
        self.vertex_num = vertex_num
        self.pos = pos
        self.graph_display = graph_display
        m = self.graph_display.canvas.create_oval(pos[0]-d, pos[1]-d, pos[0]+d, pos[1]+d, 
            fill=color, tag="moats", width=0)
        self.graph_display.CreateMoat(moat_id, d, color) # Get the animation in history
        self.m = m
        self.graph_display.canvas.lower(m, "vertices")
        try:
            self.graph_display.canvas.lower(m, "moats")
        except:
            pass

    def grow(self, new_embed_radius):
        # XXX Don't think we need this in GraphDisplay right now
        d = (new_embed_radius * self.graph_display.zoomFactor ) / 100.0 #Canvas radius!
        self.graph_display.canvas.coords(self.m,
...
class MoatedPartition:
    ...
    def __init__(self, graph, secondary_graph, graph_display, secondary_graph_display):
        self.graph = graph
        self.secondary_graph = secondary_graph
        self.graph_display = graph_display
        self.secondary_graph_display = secondary_graph_display
        ...
        for u in self.graph.vertices:
            for v in self.graph.vertices:
                if u < v:
                    tmp = (self.graph.embedding[u].x - self.graph.embedding[v].x)**2
                    tmp += (self.graph.embedding[u].y - self.graph.embedding[v].y)**2
                    self.D[(u,v)] = max(round(sqrt(tmp)), 0.0) # Enforce non-negative distances

        self.cm = ComponentMaker(self.graph, self.secondary_graph_display)
        self.components = []
        self.component = {} 
        for u in self.graph.vertices:
    ...
    def GrowMoats(self,dist):
        ...
        for C in self.components:
            for v in C.vertices:
                pt = self.graph_display.G.GetEmbedding(v)  
                x,y = self.graph_display.EmbeddingToCanvas(pt.x,pt.y)
                m = Moat(self.graph_display, (x,y), old_mr, moatColor, v, 'moat_' + str(v) + '-' + str(self.grow_round)) 
                moats.append(m)

        stepwidth = 0.05
        steps = int(round((self.mr - old_mr)/stepwidth))
        delta = (self.mr - old_mr) / steps
        
        for m in moats:
            self.graph_display.GrowMoat(m.moat_id, old_mr + steps*delta)
        for i in xrange(1, steps+1):
            for m in moats:
                m.grow(old_mr + i*delta)
            self.graph_display.update()
        self.graph_display.Wait()
    ...
    def UpdateComponents(self):
        for e in self.tightEdges: # self.tightEdges is set by GrowMoats. Call first!
            ...
            if self.component[u] is not self.component[v]:
                self.graph_display.AddEdge(u,v)
                ...
                if C is not D:
                    for u in C.vertices:
                        for v in D.vertices:
                            ...
                            if self.D[e] == 0.0:
                                tightEdges.append(e)
                                self.secondary_graph_display.AddEdge(e[0],e[1])
...
class GraphDisplay(): #object): XXX New Style classes fuck up Tkinter
    ...
    def NewMoatedPartition(self, graph, secondary_graph, graph_display, secondary_graph_display):
        # Change these argument names
        self.moated_partition = MoatedPartition(graph, secondary_graph, graph_display, secondary_graph_display)
        return self.moated_partition
